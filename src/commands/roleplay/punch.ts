...
export default new Command({
    name: 'socar',
    description: '[💥 Roleplay ] Socar algum usuário.',
    ...
    async execute({ interaction, options }) {
        ...
        if (interaction.user.id == user?.id) return interaction.editReply(`Você não pode socar a si mesmo.`);
        
        const bot = user ? user.bot : interaction.user.bot;
        if (bot) return interaction.editReply("Você não pode socar os bots.");
        ...
        const image = (await hmfull.HMtai.sfw.punch()).url
