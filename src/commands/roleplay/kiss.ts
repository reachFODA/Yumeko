...
export default new Command({
    name: 'beijar',
    description: '[💥 Roleplay ] Beije algum usuário.',
    ...
    async execute({ interaction, options }) {
        ...
        if (interaction.user.id == user?.id) return interaction.editReply(`Você não pode beijar a si mesmo.`);
        
        const bot = user ? user.bot : interaction.user.bot;
        if (bot) return interaction.editReply("Você não pode beijar os bots.");
        ...
        const image = (await hmfull.HMtai.sfw.kiss()).url
