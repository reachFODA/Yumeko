...
export default new Command({
    name: 'tapa',
    description: '[💥 Roleplay ] Dar um tapa em algum usuário.',
    ...
    async execute({ interaction, options }) {
        ...
        if (interaction.user.id == user?.id) return interaction.editReply(`Você não pode bater em si mesmo.`);
        
        const bot = user ? user.bot : interaction.user.bot;
        if (bot) return interaction.editReply("Você não pode dar um tapa os bots.");
        ...
        const image = (await hmfull.HMtai.sfw.slap()).url
