    ...
    async execute({ interaction }) {
        ...
        if (member?.dailyCooldown as number > Date.now()) {
            ...
            return interaction.editReply({
                embeds: [{ description: `Ainda falta ${FormatUtils.formatTime(calc).hours} horas, ${FormatUtils.formatTime(calc).minutes} minutos e ${FormatUtils.formatTime(calc).seconds} segundos.`, color: Colors.Blue }],
            });
        }
